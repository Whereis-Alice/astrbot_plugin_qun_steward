...
HELP_SECTIONS: tuple[tuple[str, tuple[tuple[str, str], ...]], ...] = (
    ...
    (
        ...
        (
            ...
            ("禁词禁言 <秒数>", "命中违禁词后的禁言时长"),
            ("刷屏禁言 <秒数>", "刷屏判定后的禁言时长"),
            ("投票禁言 <秒数> @某人", "管理员发起投票，群友贴 👍/👎 表情即算一票"),
            ("赞同禁言 / 反对禁言", "全体群成员可用的指令表态方式"),
        ),
    ),
    (
        "群务设置",
    ...
    (
        "群待办",
        (
            ("设群待办", "引用一条群消息后，把它加入当前群的待办列表"),
            ("群待办", "实时列出当前待办；完成 / 取消时使用这里的序号"),
            ("完成群待办 <序号>", "完成指定待办，序号来自最近一次「群待办」列表"),
            ("取消群待办 <序号>", "取消指定待办，序号来自最近一次「群待办」列表"),
        ),
    ),
    (
        "群管理策略",
        (
            ("设置加群方式 <方式>", "设置或查询自由加入、管理员审核、禁止加入、答题等规则"),
            ("设置入群问题 <问题> [|答案]", "设置入群问题；发送「清空」可移除问题和答案"),
            ("群搜索 开/关", "设置或查询群是否允许通过群指纹 / 群号被搜索"),
            ("成员邀请 <策略>", "设置成员邀请规则：关闭、需审核、直接邀请或百人以下直接邀请"),
            ("新成员历史 开/关", "设置新成员能否查看加入前的历史消息"),
        ),
    ),
    (
        "进群与退群",
        (
            ...
            ("进群黑名单 +QQ -QQ", "增删进群黑名单"),
            ("待审进群", "查看待审申请，会先和协议端对账，补上离线期间漏掉的"),
            ("批准 <序号> / 驳回 <序号> <理由>", "处理进群申请，也可直接引用申请通知"),
            ("进群禁言 <秒数>", "新成员入群后自动禁言"),
            ("进群欢迎 <欢迎语>", "兼容旧版单条欢迎语，支持 {nickname} 等占位符"),
            ("欢迎开关 [开|关]", "按群控制欢迎消息；不影响入群验证"),
            ("欢迎模板 [模板||模板]", "多条欢迎语，支持 {at}、{群名}、{人数}、{时间}"),
            ("欢迎图片 [URL或路径||...]", "欢迎语后追加一张或多张图片"),
            ("欢迎模式 [随机|顺序]", "多条模板的选取方式"),
            ("欢迎延迟 <秒数>", "稍后发送欢迎语，范围 0~300 秒"),
            ("入群验证 [开|关] [超时秒数]", "新人回复算术题后才发欢迎语"),
            ("验证动作 [踢出|踢出并拉黑|仅提醒]", "验证超时或答错达到上限后的处理"),
            ("验证次数 <0~10>", "最多答错几次，0 表示不限"),
            ("欢迎测试 / 欢迎配置", "预览当前欢迎效果或查看全部欢迎设置"),
            ("退群通知 开/关", "有人主动退群时在群里提示"),
            ("退群告别 [开|关]", "自定义主动退群告别消息；默认关闭，按群开启"),
            ("退群告别模板 [模板||模板]", "多条告别语，支持昵称、群名、人数、时间占位符"),
            ("退群告别图片 [URL或路径||...]", "告别语后追加图片，支持 URL、本地路径和 file://"),
            ("退群告别模式 [随机|顺序]", "多条告别模板的选取方式"),
            ("退群告别延迟 <秒数>", "稍后发送告别语，范围 0~300 秒"),
            ("退群告别测试 / 退群告别配置", "预览当前告别效果或查看全部告别设置"),
            ("退群拉黑 开/关", "主动退群者加入黑名单"),
        ),
    ),
    (
        ...
        (
            ...
            ("群文件直链 <路径>", "拿到文件的下载地址"),
            ("群文件容量", "已用空间、文件数与上限"),
            ("移动群文件 <路径> <目标文件夹>", "把文件挪到别的文件夹，目标写「根目录」即移出"),
            ("重命名群文件 <路径> <新名>", "改群文件名"),
            ("删除群文件夹 <名称或序号>", "删除文件夹；非空文件夹先预览，必须再发「删除群文件夹 确认」"),
            ("重命名群文件夹 <原名或序号> <新名>", "修改文件夹名称，也支持用「原名 -> 新名」写法"),
            ("整理群文件 [规则] [确认]", "按散落/过期/天数/大小筛出候选，先预览再确认删除"),
        ),
    ),
    (
        "群相册",
        (
            ...
            ("随机图 <相册名>", "从群相册里随机发一张，无需本地备份"),
            ("删相册图 [相册名] <序号>", "删掉相册里的某一张，序号见「查看群相册」"),
        ),
    ),
    (
        "群情报与声聊",
...
class ConfigFeature(Feature):
    ...
    def help_markdown(self) -> str:
        ...
        lines.append("> 群待办与群管理策略写入依赖协议端接口；当前端不支持时会明确提示，不会伪装成成功。")
        lines.append("> 非空群文件夹删除需要二次确认；确认口令默认 3 分钟内有效。")
        return "\n".join(lines)
